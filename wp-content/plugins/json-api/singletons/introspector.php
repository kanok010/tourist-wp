...
class JSON_API_Introspector {
  ...
  public function get_posts($query = false, $wp_posts = false , $post_type = null) {
    global $post, $wp_query;
    $this->set_posts_query($query,$post_type);
    $output = array();
    while (have_posts()) {
      the_post();
      if ($wp_posts) {
  ...
  protected function set_posts_query($query = false, $post_type = null) {
    ...
    if ($json_api->query->offset) {
      $query['posts_per_page'] = $json_api->query->offset;
    }
